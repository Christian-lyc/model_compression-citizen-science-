...
def prune(args):
    ...
    pruning_cfg['data'] = '/your/path/to/data.yaml' #"coco128.yaml"
...
#            unwrapped_parameters=unwrapped_parameters
        ...
        pruning_cfg['batch'] = 32
        #pruning_cfg['split']='test'
        pruning_cfg['device']=5
        pruning_cfg['epochs'] = 200
        validation_model.model = deepcopy(pruner.model) #deepcopy(model.model)

        validation_model.ckpt.update(dict(model=validation_model.model))
        del model.ckpt["ema"]
        torch.save(validation_model.model,"pruned_str_40.pt")
        validation_model.model=torch.load('pruned_str_40.pt')

        metric = validation_model.train(**pruning_cfg)
        pruning_cfg['split']='test'
        metrics1 =validation_model.val(**pruning_cfg)

        pruned_map = metrics1.box.map
        ...
        validation_model.export(format='onnx')
